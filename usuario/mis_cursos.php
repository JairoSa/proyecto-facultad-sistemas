...
// --- CONSULTA SEGURA PARA OBTENER LOS CURSOS INSCRITOS ---
$stmt = $conn->prepare(
...
<main class="container mt-5">
    ...
    <p class="lead text-muted" data-aos="fade-down">Aquí encontrarás todos los cursos en los que te has inscrito.</p>
    ...
    if (isset($_GET['mensaje'])) {
        ...
        switch ($_GET['mensaje']) {
            case 'baja_exitosa':
                $mensaje_texto = 'Te has dado de baja del curso correctamente.';
                $tipo_alerta = 'success';
                break;
            case 'error_general':
                ...
                <div class="card h-100 shadow-sm publication-card">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <?php if(!empty($curso['nombre_docente'])): ?>
                            <p class="card-subtitle mb-2 text-muted"><small>Docente: <?= htmlspecialchars($curso['nombre_docente']) ?></small></p>
                        <?php endif; ?>
                        ...
                        <a href="darse_de_baja.php?id=<?= $curso['id'] ?>" class="btn btn-outline-danger btn-sm mt-2" onclick="return confirm('¿Estás seguro de que deseas abandonar este curso?')">
                            <i class="fas fa-times-circle me-1"></i> Darse de baja
                        </a>
                    </div>
                </div>
            </div>
